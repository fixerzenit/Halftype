      ...
      >
        ...
        <div className="relative flex shrink-0 flex-wrap items-center justify-center gap-x-2 gap-y-2 sm:gap-x-3 lg:justify-between">
          <div className="order-2 flex flex-wrap items-center justify-center gap-2 lg:order-none">
            ...
            {/* Not on a phone. Recording means holding the tab open while it
                encodes, which is a desk job. */}
            {canRecord && (
              <Button
                onClick={handleVideo}
                disabled={!geo || busy}
                variant={busy === 'video' ? 'yellow' : 'fill'}
                className="hidden min-w-[4.6rem] sm:block"
              >
                {busy === 'video' ? `${Math.round(progress * 100)}%` : clipFormat.toUpperCase()}
              </Button>
            )}
          </div>
          ...
          {pattern.params.length > 0 && (
            <Button
              ...
              variant="violet"
              title="Randomise this pattern's controls"
