...
import { diffusionMarks } from './diffusion.js';
import { matrixMarks } from './matrix.js';
import { blotMarks } from './blots.js';
import { captchaMarks } from './captcha.js';
 ...
 *   see hatch() for why that matters for round caps and continuity.
 *
 * ON TONE
...
const DEFINED = [
...

  {
    id: 'rules',
    label: 'Rules',
    blurb: 'Parallel strokes across the shape — cut to it, or weighed by it.',
    motion: { key: 'angle', from: 0, to: 180, loop: 'wrap' },
    params: [
      /**
       * Two styles that were one idea.
       *
       * Hatching measures where a rule actually crosses the shape and draws
       * only those spans. Bars walks a column and turns each run of ink into
       * one weighted mark. Both are parallel strokes laid across a silhouette;
       * the difference is whether a stroke is *cut* by the shape or *sized* by
       * it, which is a choice and not a style.
       *
       * The angle is what makes them one card rather than two. Hatching always
       * had it; Bars never did, and could only ever be columns. Now it is the
       * same control on the same axis for both, so turning it is the same
       * gesture whichever you are drawing.
       */
      {
        key: 'mode',
        label: 'Stroke',
      ...
      { key: 'thickness', label: 'Line thickness', min: 0.25, max: 14, step: 0.25, def: 3.5 },
    ...
    render: ({ p, geo, fg, fx }) => {
      if (p.mode === 'cut') {
        return { body: hatch({ geo, color: fg, fx, ...p }), clip: false };
      }
...

  {
    ...
    render: ({ p, geo, fg, fx }) => {
      ...
      // One continuous coil, cut into segments only so each can carry its own
      // weight — the path itself never breaks.
      const turns = reach / p.spacing;
      const segments = Math.min(9000, Math.ceil(turns * 90));
      const parts = [];
      let prev = null;

      for (let i = 0; i <= segments; i++) {
        const t = i / segments;
        const angle = t * turns * Math.PI * 2 + p.phase * Math.PI * 2;
        const r = t * reach;
        ...
        if (prev) {
          ...
          const shown =
            ...
            !irisHides(fx, nx, ny) &&
            !(fx.build < 1 && t > fx.build);
          if (shown) {
            const gain = weighsMarks(fx) ? markWeight(fx, nx, ny) : 1;
            const w = Math.max(0.15, p.thickness * ink * Math.max(0.05, gain));
            parts.push(
              `<line x1="${num(prev[0])}" y1="${num(prev[1])}" x2="${num(x)}" y2="${num(y)}" stroke-width="${num(w)}"/>`,
            );
          }
        }
        prev = [x, y];
      ...
      return {
        body: `<g fill="none" stroke="${fg}" stroke-linecap="round">${parts.join('')}</g>`,
        clip: false,
      };
    },
  },

  {
    ...
    params: [
      ...
      { key: 'thickness', label: 'Stroke width', min: 0.25, max: 5, step: 0.25, def: 1.25 },
      SEED,
    ],
    render: ({ p, geo, fg, fx }) => ({
      body: beadMarks({ geo, fx, color: fg, ...p }),
      clip: false,
    }),
  },
...

  {
    ...
    params: [
      ...
      /**
       * How strongly the word feeds the colony — and the range is the whole
       * point of it.
       *
       ...
       * colony spreads evenly over the word and draws a thickened letter. That
       * is what this style shipped, at every setting of every other control —
       * "bridges letter to letter" was in the blurb and nowhere in the output.
       *
       ...
       * six times above the top of it. Rescaled to the band, and the old
       * behaviour is still there at the far end where it belongs.
       *
       * The default sits at the top of that band rather than the middle of it,
       * because the two things being traded are the network and the word: lower
       * and the filaments are finer but the letters stop reading, higher and
       * the letters come back as a slab. Three hundredths keeps both.
       */
...

  {
    id: 'captcha',
    label: 'Captcha',
    blurb: 'Barely bent, and eaten through by holes that were never separate.',
    skipTonal: true,
    motion: { key: 'blobs', from: 0, to: 0.7, loop: 'pingpong' },
    params: [
      // Subtle by default. The warp used to carry the whole style and had to
      // be strong to do it; with the blobs doing the damage, a light bend is
      // enough to keep the letters off their baseline, and the word stays
      // readable — which is what makes the holes read as holes rather than as
      // more of the same noise.
      { key: 'warp', label: 'Warp', min: 0, max: 0.9, step: 0.02, def: 0.1, ...RATIO },
      { key: 'scale', label: 'Warp size', min: 0.15, max: 4, step: 0.05, def: 0.7, ...RATIO },
      { key: 'blobs', label: 'Holes', min: 0, max: 1, step: 0.02, def: 0.42, ...RATIO },
      { key: 'blobSize', label: 'Hole size', min: 0.2, max: 2.2, step: 0.05, def: 0.85, ...RATIO },
      { key: 'swell', label: 'Ink bleed', min: -0.6, max: 1.2, step: 0.02, def: 0.24, ...RATIO },
      { key: 'grain', label: 'Rough edge', min: 0, max: 1.5, step: 0.02, def: 0.3, ...RATIO },
      { key: 'smoothing', label: 'Smoothing', min: 0, max: 1.5, step: 0.05, def: 0.3, ...RATIO },
      {
        key: 'style',
        label: 'Letters',
        kind: 'select',
      ...
      { key: 'thickness', label: 'Stroke width', min: 0.25, max: 8, step: 0.25, def: 2 },
      SEED,
    ],
    render: ({ p, geo, fg, fx }) => ({
      body: captchaMarks({ geo, fx, color: fg, ...p }),
      clip: false,
    }),
  },
...

  {
    id: 'turing',
    label: 'Turing',
    blurb: 'Two chemicals fed on the word, left long enough to make their own.',
    motion: { key: 'growth', from: 0.06, to: 0.85, loop: 'pingpong' },
    params: [
      /**
       * One slider for the chemistry, and it is one on purpose.
       *
       * Feed and kill are two numbers, and the patterns live on a thin curve
       * through the plane of them — so a control per axis spends most of its
       * travel outside the curve, which is exactly what the earlier
       * reaction-diffusion style here shipped before it was taken out. This
       * walks *along* the curve: every position is a named regime, and there is
       * nowhere on it that is dead, because the dead parts of the space are not
       * on the control. Tested over the whole travel at four growths — thirty
       * six settings, none dead, none flooded.
       */
      { key: 'pattern', label: 'Chemistry', min: 0, max: 1, step: 0.02, def: 0.25, ...RATIO },
      { key: 'growth', label: 'Growth', min: 0.02, max: 1, step: 0.02, def: 0.34, ...RATIO },
      { key: 'scale', label: 'Grain', min: 0.35, max: 1, step: 0.05, def: 0.8, ...RATIO },
      /**
       * Where the reaction starts, which is a different question from where it
       * is fed. Outline is the one that is not reachable any other way: the
       * letter fills from its own rim, which is the thin bulbed skeleton the
       ...
       */
      {
        ...
        def: 'fill',
        options: [
          { value: 'fill', label: 'The whole letter' },
          { value: 'edge', label: 'Its outline' },
          { value: 'scatter', label: 'Scattered' },
        ],
      },
      /**
       * How firmly the word governs where the reaction may live.
       *
       * At zero it is a dish with a letter-shaped stain in it: the pattern
       * spreads off the ink and fills the frame, and at high growth that is all
       * you get. Turned up, the desk becomes somewhere V cannot survive, so the
       * word bounds the pattern and stays a word however long it runs.
       *
       * This used to raise the feed inside the ink instead, and measuring it
       * showed it flooded the letter solid — see the note on OFF_INK_KILL in
       * diffusion.js, which has the numbers.
       */
      { key: 'grip', label: 'Hold the word', min: 0, max: 1, step: 0.04, def: 0.2, ...RATIO },
      { key: 'level', label: 'Threshold', min: 0.05, max: 0.9, step: 0.02, def: 0.4, ...RATIO },
      { key: 'smoothing', label: 'Smoothing', min: 0, max: 1.2, step: 0.05, def: 0.55, ...RATIO },
      {
        key: 'style',
        label: 'Ink',
        kind: 'select',
    ...
    render: ({ p, geo, fg, fx }) => ({
      body: diffusionMarks({ geo, fx, color: fg, ...p }),
      clip: false,
    }),
  },

  {
    ...
    motion: { key: 'glow', from: 0.1, to: 1, loop: 'pingpong' },
    params: [
      ...
      { key: 'glow', label: 'Glow', min: 0, max: 1.4, step: 0.05, def: 0.55, ...RATIO },
      { key: 'spread', label: 'Halo', min: 0.1, max: 1.6, step: 0.05, def: 0.55, ...RATIO },
    ],
    render: ({ p, geo, fg, fx }) => ({
...
const GROUPS = [
  ...
  },
  {
    ...
    ids: ['dots', 'physarum', 'turing', 'blots'],
  },
];
